...
void fcitx_client_set_capability(FcitxClient* self, guint flags)
{
    guint iflags = flags;
    if (self->priv->icproxy) {
        g_dbus_proxy_call(self->priv->icproxy, "SetCapability", g_variant_new("(u)", iflags), G_DBUS_CALL_FLAGS_NONE, -1, NULL, NULL, NULL);
    }
}
...
_fcitx_client_process_key_cb(GObject *source_object,
                             ...
                             gpointer user_data)
{
    FCITXGCLIENT_UNUSED(source_object);
...
{
    FCITXGCLIENT_UNUSED(cancellable);
    ProcessKeyStruct* pk = user_data;
    _process_key_data_free(pk);
}
...
_fcitx_client_g_signal(GDBusProxy *proxy,
                       ...
                       gpointer    user_data)
{
    FCITXGCLIENT_UNUSED(proxy);
    FCITXGCLIENT_UNUSED(sender_name);
    if (g_strcmp0(signal_name, "EnableIM") == 0) {
        g_signal_emit(user_data, signals[ENABLE_IM_SIGNAL], 0);
    }
    else if (g_strcmp0(signal_name, "CloseIM") == 0) {
        g_signal_emit(user_data, signals[CLOSE_IM_SIGNAL], 0);
    }
    else if (g_strcmp0(signal_name, "CommitString") == 0) {
