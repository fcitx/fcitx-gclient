...
static const guint32 hints_related_capability =
...
struct _FcitxIMContext {
    ...
    int cursor_pos;
    FcitxCapabilityFlags capability_from_toolkit;
    FcitxCapabilityFlags last_updated_capability;
...
static void
_fcitx_im_context_set_capability(FcitxIMContext* fcitxcontext, gboolean force);
...
fcitx_im_context_init(FcitxIMContext *context)
{
    ...
    context->last_updated_capability = CAPABILITY_SURROUNDING_TEXT;
                                  ...
                                  gint cursor_index)
{
    ...
    if (fcitx_client_is_valid(fcitxcontext->client) &&
        !(fcitxcontext->last_updated_capability & CAPABILITY_PASSWORD)) {
        gint cursor_pos;
        guint utf8_len;
        gchar *p;
...
{
    if (fcitx_client_is_valid(fcitxcontext->client)) {
        FcitxCapabilityFlags flags = fcitxcontext->capability_from_toolkit;
        ...
        if (G_UNLIKELY(update || force))
            fcitx_client_set_capability(fcitxcontext->client, fcitxcontext->last_updated_capability);
    }
}
...
_request_surrounding_text (FcitxIMContext **context)
{
    if (*context && fcitx_client_is_valid((*context)->client)) {
        ...
        if (return_value) {
            ...
            _fcitx_im_context_set_capability (*context,
                                            FALSE);
        }
        else {
...
#define CASE_PURPOSE(_PURPOSE, _CAPACITY) \
    case _PURPOSE: \
        fcitxcontext->capability_from_toolkit |= _CAPACITY; \
        break;
